 ...
 * @author Usuario
 */
public class ControladorListaCargos implements ActionListener , Runnable, MouseListener,KeyListener,MouseMotionListener{
            JFListaCargos vistaLista;
            CargosDAO modeloDAO; 
            Cargos objetoVO;
            Thread hilo=null;
            int x;
    ...
    public ControladorListaCargos(JFListaCargos vistaLista) {
            this.vistaLista = vistaLista;
            this.modeloDAO = new  CargosDAO(Conexion.f);
    ...
    public void run(){
		try {
                   ...
                   configurarTabla();
                    ...
                    List  listaObjetoVO=(List) modeloDAO.findCargoEntities();
                    LogicaGeneral.setListaEntidad("Cargos",listaObjetoVO);
                   ////[Usuario], [Tipo]
                   for(int i=0;i<listaObjetoVO.size();i++){
                        Cargos objetoVO_aux=(Cargos) listaObjetoVO.get(i);
		 ...
		 Logger.getLogger(ControladorListaCargos.class.getName()).log(Level.SEVERE, null, ex);
                 ex.printStackTrace();
    ...
    public void iniciarVistaObjetoVO(String acción){      
                ...
                if(ControladorGeneral.vistas.get("FormularioCargo")==null){
                 JFCargo vistaVO= new JFCargo();
                 ControladorCargo controladorObjetoVO =new ControladorCargo(this,vistaVO,modeloDAO,objetoVO);
                 controladorObjetoVO.acción=acción;
                 controladorObjetoVO.inicializar();
                 ControladorGeneral.setVistaJDesktopPane(vistaVO);
                 ControladorGeneral.vistas.put("FormularioCargo",controladorObjetoVO);
   ...
   //Este metodo obtiene el objetoVO selecccionado en la Tabla.
    public Cargos getObjetoVOSeleccionado(){   
     return objetoVO=(Cargos)LogicaGeneral.getListaEntidad("Cargos").get(ControladorGeneral.filaSeleccionada(select,vistaLista.tabla));
   }
    ...
    public void actionPerformed(ActionEvent e) {
            ...
            if(e.getSource()==vistaLista.btnSalir){
                 ControladorGeneral.salirVista("ListaCargos", vistaLista);
