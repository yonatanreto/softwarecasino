 ...
 * @author Usuario
 */
public class ControladorListaUsuario implements ActionListener , Runnable, MouseListener,KeyListener,MouseMotionListener{
            JFListaUsuario vistaLista;
    ...
    public ControladorListaUsuario(JFListaUsuario vistaLista) {
            this.vistaLista = vistaLista;
            this.modeloDAO = new  UsuariosDAO(Conexion.f);
    ...
    public void run(){
		try {
                   ...
                   configurarTabla();
                    ...
                    List  listaUsuarios=(List) modeloDAO.findUsuariosEntities();
                    LogicaGeneral.setListaEntidad("Usuarios",listaUsuarios);
                   ////[Usuario], [Tipo]
                   for(int i=0;i<listaUsuarios.size();i++){
                        Usuarios usuario_aux=(Usuarios) listaUsuarios.get(i);
                        Object fila[]=new Object[4];
                        fila[0]=" "+usuario_aux.getUsuario();
                        fila[1]=" "+usuario_aux.getTipo();                      
                       dtm.addRow(fila);
                   }           
		 ...
		 Logger.getLogger(ControladorListaUsuario.class.getName()).log(Level.SEVERE, null, ex);
                 ex.printStackTrace();
    ...
    public void iniciarVistaObjetoVO(String acción){      
                ...
                if(ControladorGeneral.vistas.get("FormularioUsuario")==null){
                 JFUsuario vistaUsuario= new JFUsuario();
                 ControladorUsuario controladorUsuario =new ControladorUsuario(this,vistaUsuario,modeloDAO,objetoVO);
                 controladorUsuario.acción=acción;
                 controladorUsuario.inicializar();
                 ControladorGeneral.setVistaJDesktopPane(vistaUsuario);
                 ControladorGeneral.vistas.put("FormularioUsuario",controladorUsuario);
   ...
   //Este metodo obtiene el objetoVO selecccionado en la Tabla.
    public Usuarios getUsuarioSeleccionado(){   
     return objetoVO=(Usuarios)LogicaGeneral.getListaEntidad("Usuarios").get(ControladorGeneral.filaSeleccionada(select,vistaLista.tabla));
   }
    ...
    public void actionPerformed(ActionEvent e) {
            ...
            if(e.getSource()==vistaLista.btnEditar){
                  if(!select.isSelectionEmpty()) {
                        objetoVO=getUsuarioSeleccionado();
            ...
            if(e.getSource()==vistaLista.btnSalir){
                 ControladorGeneral.salirVista("ListaUsuarios", vistaLista);
